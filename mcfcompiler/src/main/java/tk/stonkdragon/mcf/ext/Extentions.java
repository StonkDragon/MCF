...
public class Extentions extends Thread {

    private File ext;
    
    public static List<File> getExtentions() {
        List<File> e = new ArrayList<>();
        for (File f : App.EXTENTIONS.listFiles()) {
            e.add(f);
        }
        return e;
    }

    public Extentions(File ext) {
        this.ext = ext;
    }

    public void runExtention() {
        Process p;
        try {
    ...
    public void runExtention(String args) {
        Process p;
        try {
